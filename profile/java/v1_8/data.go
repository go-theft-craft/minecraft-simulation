...
//
// It hashes the numbers this profile was built from, not the file they came from:
// the block table, the motion constants, and the trigonometry table, each at the
// width the rules use. A recording that pins this can say which dataset it ran
// against, and a dataset corrected without a version bump — which is exactly what
// a physics correction looks like — stops being an unexplained digest mismatch
// and becomes a named one.
//
// The digest is computed once, when the profile is built, because a replay asks
// for it per recording and the sine table is a quarter of a megabyte.
...
func computeDataDigest(blocks blockTable, table []float32, motion map[entity.Family]sim.MotionConstants) sim.Digest {
	...
	// The block table, in handle order, which is the order it was minted in.
	writeUint(uint64(len(blocks.names)))
	for handle, name := range blocks.names {
		writeString(name)
		writeSingle(blocks.frictions[handle])
